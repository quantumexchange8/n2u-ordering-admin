...
export default function WithdrawalHistory() {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get('/transaction/getWithdrawalHistory');
    ...
    const ViewWithdrawal = (withdrawDetail) => {
        setIsOpen(true);
        setSelectedWithdrawal(withdrawDetail);
    }
    ...
    const ActionTemplate = (data) => {
        return (
            <div className="flex justify-center items-center gap-3">
                <div className="hover:rounded-full hover:bg-neutral-100 p-1 cursor-pointer" onClick={() => ViewWithdrawal(data)}>
                    <ViewDetails />
                </div>
            </div>
        );
                ...
                <div className="flex flex-col px-3 py-2">
                    {
                        selectedWithdrawal && (
                            <div className="grid grid-cols-2 gap-3 items-center ">
                                ...
                                <div className="font-bold text-sm">{selectedWithdrawal.user.name}</div>

                                <div className="max-w-20 text-sm">Wallet</div>
                                <div className="font-bold text-sm">{formatWallet(selectedWithdrawal.wallet)}</div>

                                <div className="max-w-20 text-sm">Amount</div>
                                <div className="font-bold text-sm">RM {selectedWithdrawal.amount}</div>

                                <div className="text-sm">Transaction Number</div>
                                <div className="font-bold text-sm">{selectedWithdrawal.transaction_number}</div>

                                <div className="text-sm">Payment Method</div>
                                <div className="font-bold text-sm">{selectedWithdrawal.payment_type}</div>

                                <div className="text-sm">Status</div>
                                <div className="font-bold text-sm">
                                    {
                                        selectedWithdrawal.status === 'pending' ? <Pending /> : selectedWithdrawal.status === 'success' ? <Success /> : <Rejected />
                                    }
                                </div>
                                {
                                    selectedWithdrawal.status === 'rejected' && (
                                        <>
                                            ...
                                            <div className="font-bold text-sm">{selectedWithdrawal.remark ? selectedWithdrawal.remark : '-'}</div>
                                        </>
                                    )
                                }

                                <div className="text-sm">Requested Date</div>
                                <div className="font-bold text-sm">{formatDateTime(selectedWithdrawal.created_at)}</div>

                                <div className="text-sm">Transaction Date</div>
                                <div className="font-bold text-sm">{formatDateTime(selectedWithdrawal.transaction_date)}</div>
                            </div>
                        )
                    }
                </div>
